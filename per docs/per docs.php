...
//--- 3: using ($user = Auth::login($user);) in resgister contterler to make the user authenticated (logined)
    and save the user data in the user sesstion (you can hide some user data to store in its model with protected $hidden = [}) 
        ...
        Auth::login($user);
        //with gurad (Auth::guard('patient')->login($user);)
...
some important funcs:
    ...
    4- Auth::user()->id; //to get specific data in authenticated user info saved
        - Auth::guard('patient')->id(; //in case of using the gurad
    ...
    12- @guest('patient') @endguest //to speiciy the not authiticated users
...
================ select data ================
    ...
    =Patient::where('phone_number', 'like', '%' . $search_query . '%')
...
(use Illuminate\Support\Facades\Validator;)
    ...
    $rules = [
        ...
        'email' => ['nullable', 'email:rfc,dns', ($this->email === $this->old_email) ? '' : 'unique:patients,email', 'max:255'],  //(nullable) to accept empty input  
