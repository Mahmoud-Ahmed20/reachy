...
@section('title', 'Brand | Proxima - Medical Management app')

@section('title-topbar', 'Brand')
    ...
    <div class="container-fluid px-2 mt-3">
        ...
        <div class="card shadow mb-3 pb-2">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 fw-bold text-gray-500"><i class="fas fa-globe-europe"></i> All Brands</h6>
            </div>
                    ...
                    <table class="table display datatable-modal" id="p-lab-table" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach ($brands as $item)
                                <tr>
                                    <td><img class="rounded-circle avatar-small me-2"src="{{ URL::asset('img/brands/' . $item->logo) }}"></td>
                                    <td>{{$item->name_ar}}</td>
                                    <td>{{$item->name_en}}</td>
                                    <td>{{$item->slug_ar}}</td>
                                    <td>{{$item->slug_en}}</td>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('sett.brand.edit', $item->id) }}"
                                        ...
                                        <a class="btn btn-sm modal-effect status-col-link cancel-color-btn b-r-xs mb-1 delete-conf"
                                            title="delete" data-effect="effect-scale" data-brand_id="{{ $item->id }}"
                                            data-name_ar="{{ $item->name_ar }}" data-bs-toggle="modal"
                                            data-bs-target="#delete"><i class="fas fa-trash"></i> Delete
                                        </a>
                                    ...
                                    <div class="modal fade" id="delete" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable ">
                                            <div class="modal-content b-r-s-cont border-0">
                                                ...
                                                <form action="{{ route('sett.brand.destroy', $item->id) }}" method="post">
                                                    {{ method_field('delete') }}
                                                    {{ csrf_field() }}
