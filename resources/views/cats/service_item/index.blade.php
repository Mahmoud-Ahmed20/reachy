...
@section('title', 'Services | Proxima - Medical Management app')

@section('title-topbar', 'Services')
    ...
    <div class="container-fluid px-2 mt-3">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-3">
            <span class="mb-0">
                ...
                <a class="text-gray-300">{{ __('patientappo.service controll') }}</a>
            </span>

            <div class="d-flex justify-content-center">
                <a href="{{ route('sett.service_item.create') }}"
        ...
        <div class="card shadow mb-3 pb-2">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 fw-bold text-gray-500"><i
                        class="fas fa-capsules me-2"></i>{{ __('patientappo.service controll') }}</h6>
                    ...
                    <table class="table display datatable-modal" id="p-lab-table" width="100%" cellspacing="0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-xxs">{{ __('basic.branch') }}</th>
                                <th class="text-xxs">{{ __('basic.type') }}</th>
                                <th class="text-xxs">{{ __('basic.name') }}</th>
                                <th class="text-xxs">{{ __('basic.price') }}</th>
                                <th class="text-xxs">{{ __('basic.pulses') }}</th>
                                <th class="text-xxs">{{ __('basic.package') }}</th>
                                <th class="text-xxs">{{ __('basic.package items number') }}</th>
                            ...
                            @foreach ($service as $iteam)
                                <tr>
                                    ...
                                    <td>
                                        @if ($iteam->package == 0)
                                            Not package
                                        @elseif ($iteam->package == 1)
                                            session package
                                        @elseif ($iteam->package == 2)
                                            pulses package
                                        @endif
                                    </td>

                                    <td>
                                        {{ $iteam->package_items_number }}
                                    </td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <div class="d-flex justify-content-center">
                                            @if (in_array($iteam->id, [1, 2, 3, 4]))
                                                <a href="{{ route('sett.service_item.edit', $iteam->id) }}"
        ...
        <div class="modal fade" id="delete1" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable ">
                <div class="modal-content b-r-s-cont border-0">
                    ...
                    <form action="{{ route('sett.service_item.destroy', 'test') }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
